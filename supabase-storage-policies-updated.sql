-- supabase-storage-policies-updated.sql
-- Updated Storage policies for Property Snap feature
...
-- More flexible policy for uploading property images
CREATE POLICY "Users can upload property images" ON storage.objects
FOR INSERT WITH CHECK (
  bucket_id = 'shopimage' 
  AND starts_with(name, 'property-snap/')
  AND (
    -- Allow if user is authenticated (any authenticated user can upload to property-snap)
    auth.uid() IS NOT NULL
    OR
    -- Allow if the path structure is correct (fallback for server-side uploads)
...
FOR SELECT USING (
  ...
  AND (
    ...
    OR
    -- Allow viewing if user is authenticated (for property-snap folder)
    (auth.uid() IS NOT NULL AND starts_with(name, 'property-snap/'))
  )
);
