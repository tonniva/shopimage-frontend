    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-blue-50 to-white">
        ...
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="text-center">
            ...
            <p className="text-gray-600">
              ลบพื้นหลังรูปภาพด้วย AI อัปโหลดหลายรูปพร้อมกัน
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-sm">PNG (พื้นหลังโปร่งใส)</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="radio"
                        ...
                        className="w-4 h-4 text-cyan-600"
                      />
                      <span className="text-sm">JPG (พื้นหลังสีทึบ)</span>
                    </label>
                  </div>
                </div>
            ...
            >
              {isProcessing ? (
                <>
                  ...
                  กำลังประมวลผล... ({processingProgress}%)
                </>
              ) : (
                <>
                  <Scissors size={20} />
                  ลบพื้นหลัง ({uploadedImages.length} รูป)
                </>
              )}
            </button>
          ...
          <div className="space-y-6">
            <div className="bg-white border-2 border-black rounded-xl p-6">
              <h3 className="text-lg font-bold mb-4">
                รายการรูปภาพ ({uploadedImages.length})
              </h3>
              ...
              {uploadedImages.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>ยังไม่มีรูปภาพ</p>
                  <p className="text-sm">อัปโหลดรูปเพื่อเริ่มต้น</p>
                </div>
              ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {image.status === "pending" && (
                            <span className="text-xs text-gray-500">รอประมวลผล</span>
                          )}
                          {image.status === "processing" && (
          ...
          <div className="space-y-6">
            <div className="bg-white border-2 border-black rounded-xl p-6">
              <h3 className="text-lg font-bold mb-4">ดาวน์โหลด</h3>

              {processedImages.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  <Download size={48} className="mx-auto mb-2 opacity-50" />
                  <p>ยังไม่มีรูปที่ประมวลผลแล้ว</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {/* Summary */}
                  <div className="bg-gradient-to-br from-cyan-50 to-blue-50 p-4 rounded-lg border-2 border-cyan-200">
                    <div className="flex justify-between mb-2">
                      <span className="text-sm font-semibold">ประมวลผลสำเร็จ:</span>
                      <span className="text-sm font-bold text-cyan-600">
                        {processedImages.length} รูป
                      </span>
                    </div>
                  </div>
                  ...
                  {/* Preview Grid */}
                  <div>
                    <h4 className="text-sm font-semibold mb-3">ตัวอย่างรูปที่ประมวลผลแล้ว:</h4>
                    ...
                    {processedImages.length > 4 && (
                      <p className="text-xs text-gray-500 mt-2 text-center">
                        +{processedImages.length - 4} รูปเพิ่มเติม
                      </p>
                    )}
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Before */}
              <div>
                <p className="text-sm font-semibold mb-2 text-gray-700">ก่อน:</p>
                <img
              ...
              {/* After */}
              <div>
                <p className="text-sm font-semibold mb-2 text-gray-700">หลัง:</p>
