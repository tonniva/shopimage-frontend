...
const getPageSource = () => {
  ...
  const pageMap = {
    ...
    '/property-snap': 'PROPERTY_SNAP',
    '/en/property-snap': 'PROPERTY_SNAP_EN',
    '/property-snap/create': 'PROPERTY_SNAP_CREATE',
    '/en/property-snap/create': 'PROPERTY_SNAP_CREATE_EN',
...
export const trackPropertySnap = {
  // Main actions
  create: (reportId, imageCount, placeCount) => trackFeatureEvent('PROPERTY_SNAP', 'report_create', { label: `images_${imageCount}_places_${placeCount}`, value: imageCount }),
  update: (reportId, imageCount, placeCount) => trackFeatureEvent('PROPERTY_SNAP', 'report_update', { label: `images_${imageCount}_places_${placeCount}`, value: imageCount }),
  share: (reportId, platform) => trackFeatureEvent('PROPERTY_SNAP', 'report_share', { label: platform, value: 1 }),
  view: (reportId, isOwner) => trackFeatureEvent('PROPERTY_SNAP', 'report_view', { label: isOwner ? 'owner' : 'public', value: 1 }),
  
  // Step tracking
  upload: (imageCount, totalSize) => trackFeatureEvent('PROPERTY_SNAP', 'image_upload', { label: `count_${imageCount}`, value: imageCount }),
  location: (lat, lng, hasAddress) => trackFeatureEvent('PROPERTY_SNAP', 'location_select', { label: hasAddress ? 'with_address' : 'coordinates_only', value: 1 }),
  places: (placeCount) => trackFeatureEvent('PROPERTY_SNAP', 'places_found', { label: `count_${placeCount}`, value: placeCount }),
  
  // Social sharing
  shareFacebook: (reportId) => trackFeatureEvent('PROPERTY_SNAP', 'share_facebook', { label: reportId, value: 1 }),
  shareLine: (reportId) => trackFeatureEvent('PROPERTY_SNAP', 'share_line', { label: reportId, value: 1 }),
  shareCopy: (reportId) => trackFeatureEvent('PROPERTY_SNAP', 'share_copy', { label: reportId, value: 1 }),
  
  // Navigation
  openMaps: (reportId) => trackFeatureEvent('PROPERTY_SNAP', 'open_maps', { label: reportId, value: 1 }),
  downloadImages: (reportId, imageCount) => trackFeatureEvent('PROPERTY_SNAP', 'download_images', { label: `count_${imageCount}`, value: imageCount }),
  
  // Property Search tracking
  searchPageView: (category, province) => trackFeatureEvent('PROPERTY_SEARCH', 'page_view', { label: `${category}_${province}`, value: 1 }),
  search: (category, province, searchTerm, resultCount) => trackFeatureEvent('PROPERTY_SEARCH', 'search', { label: `${category}_${province}_${searchTerm || 'no_term'}`, value: resultCount }),
  viewModeChange: (mode) => trackFeatureEvent('PROPERTY_SEARCH', 'view_mode', { label: mode, value: 1 }),
  propertyClick: (propertyId, category, province) => trackFeatureEvent('PROPERTY_SEARCH', 'property_click', { label: `${category}_${province}`, value: 1 }),
  pagination: (page, category, province) => trackFeatureEvent('PROPERTY_SEARCH', 'pagination', { label: `${category}_${province}_page_${page}`, value: page }),
  shareFromSearch: (propertyId, category, province) => trackFeatureEvent('PROPERTY_SEARCH', 'share', { label: `${category}_${province}`, value: 1 }),
  phoneClick: (propertyId, category, province) => trackFeatureEvent('PROPERTY_SEARCH', 'phone_click', { label: `${category}_${province}`, value: 1 })
};
